"""Main doclinks orchestrator - ties all librarian scripts together."""
...
def cmd_build(args: argparse.Namespace) -> int:
    """Build complete doc links (index, extract, resolve)."""
    root = Path(args.root) if args.root else Path.cwd()

    print("=== Building Doc Links ===\n")
    ...
    print("=== Doc Links Build Complete ===")
    return 0
...
def cmd_check(args: argparse.Namespace) -> int:
    ...
    if links_index is None:
        print("Error: Links index not found. Run 'doclinks build' first.")
        return 1
...
def cmd_status(args: argparse.Namespace) -> int:
    """Show doc links status."""
    root = Path(args.root) if args.root else Path.cwd()

    print("=== Doc Links Status ===\n")
    ...
    if not symbol_index or not links_index:
        print("Run 'doclinks build' to create doc links.")
    elif links_index["total_broken"] > 0 or links_index["total_errors"] > 0:
        print("Issues found. Run 'doclinks fix' to generate fix prompts.")
    else:
        print("Doc links are healthy.")
...
def cmd_fix(args: argparse.Namespace) -> int:
    ...
    if report["total_issues"] == 0:
        print("No issues to fix. Doc links are up to date.")
        return 0
...
def cmd_rebuild(args: argparse.Namespace) -> int:
    """Rebuild doc links from scratch."""
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(
        description="Doclinks - Documentation reference management",
        formatter_class=argparse.RawDescriptionHelpFormatter,
        epilog="""
Commands:
  build     Build complete doc links (index, extract, resolve)
  check     Check links for staleness
  status    Show doc links status
  fix       Generate fix prompts for issues
  rebuild   Rebuild doc links from scratch

Examples:
  python -m scripts.librarian.doclinks build
  python -m scripts.librarian.doclinks check
  python -m scripts.librarian.doclinks status
