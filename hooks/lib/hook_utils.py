...
def init_context(log_file: Optional[str] = None) -> HookContext:
    ...
    """
    ctx = HookContext(
        log_file=log_file if log_file is not None else (os.environ.get("HEO_LOG_FILE") or os.environ.get("FROSTY_LOG_FILE")),
    )
    _hook_context.set(ctx)
    return ctx
    ...
    def decorator(func):
        ...
        def wrapper(*args, **kwargs):
            ...
            try:
                ...
                if "--debug" in sys.argv or os.environ.get("HEO_DEBUG") or os.environ.get("FROSTY_DEBUG"):
