...
def is_heo_project(project_dir: Optional[Path] = None) -> Tuple[bool, str]:
    ...
    # Check environment variable override (with backwards compat)
    if os.environ.get("HEO_FORCE_ENABLE") or os.environ.get("FROSTY_FORCE_ENABLE"):
...
def is_safe_project_scope(project_dir: Optional[Path] = None) -> Tuple[bool, str]:
    ...
    if os.environ.get("HEO_DISABLE_HOOKS") or os.environ.get("FROSTY_DISABLE_HOOKS"):
        return False, "HEO_DISABLE_HOOKS set"

    # Check for skip file BEFORE accepting project result
    # This allows per-project disable even for heo-compatible projects
    for skip_name in [".heo-skip-hooks", ".frosty-skip-hooks"]:
        skip_file = project_dir / skip_name
        if skip_file.exists():
            return False, f"{skip_name} file exists"
...
def log_diagnostic(message: str, **data) -> None:
    ...
    if os.environ.get("HEO_DEBUG") or os.environ.get("FROSTY_DEBUG"):
...
is_frosty_project = is_heo_project
