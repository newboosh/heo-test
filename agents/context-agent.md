...
| **Librarian Doc Links** | Relevant symbols, file locations, doc-to-code references (received FROM Librarian) |
...
| **Librarian** | Doc links info — the Librarian owns the doc links and passes relevant info to Context Agent |

**Note:** The Librarian owns `symbols.json`, `links.json`, and `fix_report.json`. The Context Agent does not read these directly. The Librarian provides relevant doc links information when invoked.
...
Task
 ...
 ├─► Librarian             → Receives doc links info (symbols, file locations, link states)
...
Explore is a **fallback** for unfamiliar territory. For known code areas, the Librarian's doc links provide faster, indexed lookups.

## Relationship to Other Agents

### Librarian Agent
The **Librarian** owns the doc links (`symbols.json`, `links.json`, `fix_report.json`):
- Librarian reads the doc links and passes relevant info to Context Agent
- Context Agent flags stale/broken doc references back to Librarian
- Context Agent does NOT read doc links files directly
...
When spawned as a **team teammate** (via Task tool with `team_name` parameter):

### Delivery Adaptation
- **Initial briefing:** Broadcast SUMMARY-tier context to all teammates via `SendMessage` (type: `broadcast`)
- **On-demand RAW:** Deliver full RAW-tier content via direct `SendMessage` when a teammate sends a `CONTEXT_REQUEST`
- **Follow-up queries:** Remain alive and responsive for the team's duration — do NOT exit after initial delivery

### Librarian Integration
- Librarian remains **your subagent** (spawned via Task tool within your own context)
- Librarian is NOT promoted to a separate teammate — you own the doc-links relationship
- Request doc-links data from Librarian the same way as in solo mode

### Message Protocol

**Inbound (from teammates):**
```
CONTEXT_REQUEST: <what they need>
```
Respond with targeted RAW-tier content via direct message.

**Outbound (to teammates):**
```
CONTEXT_BRIEFING: <structured summary>
```
Broadcast once after initial gathering completes.

### Differences from Solo Mode
| Aspect | Solo Mode | Team Mode |
|--------|-----------|-----------|
| Delivery | Write hybrid payload document | Broadcast SUMMARY, direct-message RAW |
| Lifecycle | Gather → deliver → exit | Gather → broadcast → remain for queries |
| ACK | Receiver sends PAYLOAD_ACK | Not needed (SendMessage confirms delivery) |
| Fingerprint | Included in payload META | Included in broadcast for reference |
...
- Receive doc links info from Librarian—don't read doc links files directly
